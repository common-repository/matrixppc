...
class MatrixPPC_Utils
{
    const MATRIXPPC                             =   "matrixppc";
    const MATRIXPPC_API_VERSION                 =   "1.0";
     ...
     * @access  public
     */
    public static $actionsId = array( "bold" => 1 );

    public static $boldTags=array("b","strong");
    ...
    public static function checkActiveAlgos(){
        $numActive = 0;
        $algos = array(
            'mx_ppc_algo_bold',
            'mx_ppc_algo_fraud'
        ...
        foreach($algos as $algo){
            if(MatrixPPC_Config::get($algo) != 0){
                $numActive+=1;
            }
        }
        return $numActive;
    ...
    public static function writeLogRow($type, $log_row){
        ...
        self::appendLogRow($full_path, $log_row, MatrixPPC_Config::get("mx_max_filesize"));
    }
    ...
    /**
     * This function writes to the debug file.
     * @since   1.0.0
     * @access  public
     ...
     * @return void
     */
    public static function cronDebug($what, $level=1){
        if(
                MatrixPPC_Config::get('mx_ppc_activate_cronlog') == '1' ||
                MatrixPPC_Config::get('mx_ppc_activate_cronlog') === false
          ){        	
          	if( MatrixPPC_Config::get('mx_ppc_debug_level') >= $level ){
          		self::setSafeFileContents(self::getStorageDirectory("debug.php"),date("Y-m-d H:i:s").": ".self::getDebugBacktrace().$what."\n",true);
          	}
        }
    }

    /**
     * This function gets the IPs of Search Engines from the file.
     * @since   1.0.0
     * @access  public
     * @param   void
    ...
    /**
     * This function gets the Search Engines Referrers that matches from the file.
     * @since   1.0.0
     * @access  public
     * @param   void
    ...
    /**
     * This function gets the visitors IP.
     * @since   1.0.0
     * @access  public
     * @param   void
     * @return  bool
    ...
    /**
     * This function gets the visitors referer.
     * @since   1.0.0
     * @access  public
     * @param   void
     * @return  bool
    ...
    /*public static function isIpInRange($ip, $range ) {
        if(filter_var($range, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6)){
            return false;//TODO: This is just a hack until we implement inet_pton
        }
        if ( strpos( $range, '-' ) ){
    ...
    public static function setPageContent($url, $ip, $extra){
        ...
        $writeThis=MatrixPPC_Utils::getJSONFromArray($data);
    ...
    public static function setSafeFileContents($dir, $contents, $append=false){
        ...
        if(!file_exists($dir)) {
            ...
            if (!is_dir(dirname($dir))) {
                mkdir(dirname($dir), 0775, true);//TODO: should we use 0775 ?
            }
        }
        else{
            if(!$append){
    ...
    public static function getStorageDirectory($item = ''){
        return MatrixPPC_Db::getBaseStoragePath($item);
    }
    ...
    public static function getActionsDirectory($item = ''){
        return MatrixPPC_Db::getBaseStoragePath('actions', $item);
    }

    /**
     * This function gets the path to the ips directory.
     * @since   1.0.0
     * @access  public
     * @param   string $item
     * @return  string
     */
    public static function getIPsDirectory($item = ''){
        return MatrixPPC_Db::getBaseStoragePath('ip_files', $item);
    }
    ...
    public static function getSearchEnginesDirectory($item = ''){
        return MatrixPPC_Db::getBaseStoragePath('s_files', $item);
    }
    ...
    public static function getUrlsDirectory($item = ''){
        return MatrixPPC_Db::getBaseStoragePath('urls', $item);
    }
    ...
    public static function getDebugBacktrace() {
        ...
        $c = '';
		if(MatrixPPC_Config::get('mx_ppc_debug_level')  == 2){
        	$c .= $file . ": ";
        }
        elseif(MatrixPPC_Config::get('mx_ppc_debug_level')  == 3){
        	$c .= $file . ": ";
        	$c .= ($class != '') ? $class . "->" : "";
        	$c .= ($func != '') ? $func . "(): " : "";
        }
    ...
    public static function deleteFile($file){
        if (file_exists($file)) {
            unlink($file);
            MatrixPPC_Utils::cronDebug("Deleted " . $file . ". ", 2);
        }
    }
    ...
    public static function deleteActionsFiles($dir = false){
        ...
        if($dir != MatrixPPC_Utils::getActionsDirectory()){
            return rmdir($dir);
        }
    }
    ...
    public static function debugTail(){
	    $readSize=5024 * 10;
	    $debugFile=MatrixPPC_Utils::getStorageDirectory('debug.php');
    ...
    public static function buildSyncDataPackage($max_data_size = 8000000){
        ...
        $maxConfigSize = MatrixPPC_Config::get("mx_max_send_size");
        ...
        $all_files = array(
            's_files' => array_diff(glob(self::getSearchEnginesDirectory('*.php')),array(MatrixPPC_Utils::getSearchEnginesDirectory("index.php"))),
        );
        ...
        MatrixPPC_Config::updateThrottle($throttleCall);
